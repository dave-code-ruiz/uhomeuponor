...
from homeassistant.const import ATTR_ATTRIBUTION, CONF_NAME, CONF_HOST, TEMP_CELSIUS
...
def setup_platform(hass, config, add_entities, discovery_info=None):
    ...
    _LOGGER.info("start setup platform sensor Uhome Uponor")
    ...
    add_entities([UHomeSensor(host, uhome, thermostat)
                  for thermostat in uhome.uhome_thermostats], True)


class UHomeSensor(Entity):
    """Representation of a Sensor."""

    def __init__(self, host, uhome, thermostat):
    ...
    def name(self):
        ...
        return 'Uhome ' + str(self.thermostat.uhome_thermostat_keys['room_name']['value'])
    ...
    def device_state_attributes(self):
        ...
        return {
            ATTR_ATTRIBUTION: self.thermostat.uhome_thermostat_keys['regulation_mode']['value'],
        }
    ...
    def update(self):
        ...
        """
        self.__update_keys(self.thermostat.uhome_thermostat_keys)

    def __do_rest_call(self, data):
        uri = 'http://' + self.uhome.IP + '/api'
        response_data = ''
        try:
            response = requests.post(uri, data=data)
            response_data = json.loads(response.text)
        except requests.exceptions.ConnectionError:
            print("Connection to Uhome unit on IP address: " + self.uhome.IP + " failed.")
