...
class Uhome(object):
    ...
    def set_thermostat_value(self, thermostat, name, value):
        for key_name, key_data in thermostat.uhome_thermostat_keys.items():
            if key_name == name:
                ...
                response_data = self.do_rest_call(data)
                try:
                    _LOGGER.info("value set to " + str(value)  + " via Rest Full Api, response: " + str(response_data['result']))
                except KeyError:
                    pass
                except IndexError:
                    pass
    ...
    def set_hc_mode(self, thermostat, hvac_mode):
        name = ''
        value = ''
#        self.set_thermostat_value(thermostat, name, value)

    def set_preset_mode(self, thermostat, preset_mode):
        name = ''
        value = ''
#        self.set_thermostat_value(thermostat, name, value)
